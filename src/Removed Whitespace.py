from collections import deque;import time as tm;R=4;C=3;c=[[1,2,0,1],[0,0,0,0],[1,4,0,1],[0,3,3,0],[3,0,0,3]]
d=[["   ","   "],["┌–┐","| |","| |","└–┘",],["┌––––┐","|    |","|    |","└––––┘",],["┌–┐","└–┘",],["┌––––┐","└––––┘"]]
r=print;n=range;e=enumerate;tp=tuple;z=list;t=int;
def h(c:z,i)->tp:
    return tp(tp(q) for q in c) if not i else [z(map(t,q)) for q in c]
def p(c:z)->None:
    r("\n");nb=[[" "] * 12 for _ in n(10)]
    for q in n(R+1):
        for x in n(C+1):
...
def m(c:z,g:z)->z:
    ...
    return c
def ds(c:z):
    c=[z(map(lambda x:x if x>0 else 0,c[i])) for i in n(len(c))]
    for q,i in e(c):
        for x,t in e(i):
...
if this!=None:
    ...
    for i in this:input("Press Enter for next g: \n");r(f"Move: {i[2]}");p(h(i[0],1))